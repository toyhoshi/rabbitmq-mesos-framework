...
class MarathonObject:
    ...
    def __init__(self, dcos_url):
        logging.info("*** Initializing Marathon Scheduler...")

        if dcos_url[-1] == "/":
            self.dcos_url = dcos_url[:-1]
        else: self.dcos_url = \
            dcos_url

        self.marathon_url = dcos_url + ":8080"
        logging.info("Marathon's url set to %s" %self.marathon_url)
        ...
        logging.info("Wait some seconds for MesosDNS update...")
        time.sleep(30)
        ...
        for key in outp:
            if (key['ip'] == os.environ["HOST"]) & (key['port'] == "5672"):
                os.system('hostname '+ key['host'][:-1])

        # Dummy authentication
        self.marathon_auth = ("","")
        # logging.info("Marathon's HTTP authentication info set to %s" %self.marathon_auth)

        self.mesos_url = dcos_url + "/mesos"
        logging.info("Mesos master's url set to %s" %self.mesos_url)
    ...
    def get_app_tasks(self):
        ...
        try:
            ...
            if r.status_code>=200 and r.status_code<300:
                ...
                logging.info("Found the following tasks:")
                [logging.info(item) for item in tasks]
                if len(tasks) != 0:
